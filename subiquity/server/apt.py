...
class AptConfigurer:
        ...
        if self.app.base_model.network.has_network:
            with contextlib.suppress(FileNotFoundError):
                self.install_tree.pp("etc/apt/sources.list").rename(
                    self.install_tree.pp("etc/apt/sources.list.d/10sy-original.list"),
                )
        else:
    ...
    async def deconfigure(self, context, target: str) -> None:
        ...
        target_mnt.pp("etc/apt/sources.list.d/10sy-original.list").unlink(
            missing_ok=True
        )
        target_mnt.pp("etc/apt/sources.list.d/01sy-cdrom.list").unlink()
        with contextlib.suppress(FileNotFoundError):
