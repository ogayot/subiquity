...
class MountList(WidgetWrap):
    ...
    def refresh_model_inputs(self):
        mountinfos = [
            MountInfo(mount=m)
            for m in sorted(
                self.parent.model.all_mounts(), key=lambda m: (m.path == "", m.path)
            )
        ]
        if len(mountinfos) == 0:
            ...
            if path_markup == "":
                ...
                for j in range(i - 1, -1, -1):
                    ...
                    if mi.startswith(mi2):
                        ...
                        part2 = "/".join([""] + mi.split_path[len(mi2.split_path) :])
...
class WhyNotStretchy(Stretchy):
    def __init__(self, parent, obj, action, whynot):
        ...
        title = "Cannot {action} {type}".format(
            action=_(action.value).lower(), type=labels.desc(obj)
        )
        widgets = [
            Text(whynot),
            Text(""),
...
class DeviceList(WidgetWrap):
    ...
    _lvm_partition_EDIT = _stretchy_shower(
        lambda parent, part: PartitionStretchy(parent, part.volgroup, partition=part)
    )
    ...
    def _action_menu_for_device(self, device):
        ...
        for action in DeviceAction.supported(device):
            ...
            if not whynot and action in [DeviceAction.DELETE, DeviceAction.REFORMAT]:
    ...
    def refresh_model_inputs(self):
        devices = [
            d
            for d in self.parent.model.all_devices()
            if (
                d.available() == self.show_available
...
class FilesystemView(BaseView):
    ...
    def __init__(self, model, controller):
        ...
        self._create_vg_btn = Toggleable(
            menu_btn(label=_("Create volume group (LVM)"), on_press=self.create_vg)
        )
        ...
        super().__init__(
            screen(
                self.lb, self._build_buttons(), focus_buttons=self.model.can_install()
            )
        )
    ...
    def refresh_model_inputs(self):
        ...
        if guidance:
            self.frame.contents.insert(0, (guidance, self.frame.options("pack")))
