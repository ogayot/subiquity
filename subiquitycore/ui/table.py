...
class TableRow(WidgetWrap):
    ...
    def adjust_for_spanning_cells(
        self, unpacked_user_indices, no_inherent_size, widths
    ):
...
def _compute_widths_for_size(maxcol, table_rows, colspecs, default_spacing):
    ...
    unpacked_user_indices = {user_i for user_i, cs in colspecs.items() if not cs.pack}
    ...
    for user_i in range(colcount):
        if user_i not in unpacked_user_indices and widths.get(2 * user_i, 0) == 0:
            no_inherent_size.add(user_i)
            widths[2 * user_i] = 0

    # Set the widths of the spacing columns.
    widths.update({2 * user_i + 1: default_spacing for user_i in range(colcount - 1)})
    widths.update(
        {
            2 * user_i + 1: cs.rpad
            for user_i, cs in colspecs.items()
